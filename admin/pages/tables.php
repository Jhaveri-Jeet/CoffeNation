...
<div class="container-fluid">
                            ...
                            <tbody>
                                <?php for ($i = 0; $i < count($data); $i++) { ?>
                                    <tr>
                                        ...
                                        <td class="border-bottom-0">
                                            <a class="mb-0 fw-normal align-middle" onclick="showUpdateModal(<?= $data[$i][0] ?>, '<?= $data[$i][1] ?>', '<?= $data[$i][2] ?>')">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-edit" onclick="showUpdateModal(<?= $data[$i][0] ?>, '<?= $data[$i][1] ?>', '<?= $data[$i][2] ?>')" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                        ...
                                        <td class="border-bottom-0">
                                            <a class="mb-0 fw-normal align-middle" href="<?= urlOf("admin/assets/api/deleteTable.php?id=") . $data[$i][0] ?>">
    ...
    <dialog id="insertTableModal" style="top: 50%;left: 50%;border:0px;border-radius:10px; height: 50%;width: 30%;-webkit-transform: translateX(-50%) translateY(-50%);-moz-transform: translateX(-50%) translateY(-50%);-ms-transform: translateX(-50%) translateY(-50%);transform: translateX(-50%) translateY(-50%);">
        <form method="POST" action="<?= urlOf("admin/assets/api/insertTable.php") ?>">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label mb-3">Table Number : </label>
                        <input type="number" class="form-control" name="tableNumber" placeholder="Enter Table Number : ">
                    </div>
    ...
    <dialog id="updateTableModal" style="top: 50%;left: 50%;border:0px;border-radius:10px; height: 50%;width: 30%;-webkit-transform: translateX(-50%) translateY(-50%);-moz-transform: translateX(-50%) translateY(-50%);-ms-transform: translateX(-50%) translateY(-50%);transform: translateX(-50%) translateY(-50%);">
        <form method="POST" action="<?= urlOf("admin/assets/api/updateTable.php") ?>">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label mb-3">Update Table Number : </label>
                        <input type="text" class="form-control" name="updateTableNumber" id="tableNumber">
                        <input type="hidden" class="form-control" name="updateId" id="id">
                    </div>
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label mb-3">Update Table Status : </label>
